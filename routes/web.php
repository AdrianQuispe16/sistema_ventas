...
Route::prefix('ventas')->group(function () {
    ...
    Route::get('/agregar-al-carrito/{id_producto}', [Ventas::class, 'agregarAlCarrito'])->name('ventas.agregar.al.carrito');
    Route::get('/borrar-carrito', [Ventas::class, 'borrar_carrito'])->name('ventas.borrar.carrito');
    Route::get('/quitar-del-carrito/{id_producto}', [Ventas::class, 'quitarDelCarrito'])->name('ventas.quitar.del.carrito');
    Route::post('/guardar-venta', [Ventas::class, 'guardarVenta'])->name('ventas.guardar.venta');
});

Route::prefix('detalles')->middleware('auth')->group(function () {
    Route::get('/detalles-venta', [DetalleVentas::class, 'index'])->name('detalles-venta');
    Route::get('/detalle/{id}', [DetalleVentas::class, 'detalle'])->name('detalle.venta');
    Route::delete('/revocar/{id}', [DetalleVentas::class, 'revocar'])->name('detalle.venta.revocar');
    Route::get('/ticket/{id}', [DetalleVentas::class, 'ticket'])->name('ticket.venta');
});
...
Route::prefix('usuarios')->middleware('auth','Checkrol:admin')->group(function () {
    ...
    Route::get('/cambiar-password/{id}/{password}', [Usuarios::class, 'cambiar_Password'])->name('cambiar.password');
});
